...
def display_overview():
    ...
    :return: None
    """
    print("The goal of this homework is to keep asking the user for a number until the letter 's' is provided.")
    print("Each number provided will be stored in a list and used to calculate the average using a loop.")
...
#   - !success
#   - illogical code that takes longer than 10 seconds to understand
def is_float(an_input="five"):
    """
    Use try/except to attempt to convert supplied input into a float otherwise display an error
    :return: None
    """
    try:
        # checks if the input can be converted to a floating point number
        float(an_input)
        # if it can then the converted parameter is returned
        return True
    # if the supplied parameter can not be converted to a float
    # then, execute this block
    except ValueError:
        print(f"{an_input} can not be converted to a floating point number. Please enter any number.")
        return False
